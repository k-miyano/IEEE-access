...
Therefore, this paper proposes a scheduling method of multi-UAV search system that considers processing time of image data and data transfer time.
We present the utility-based problem formulation that ensures the freshness of individual obtained piece of information while obtaining as many pieces of information as possible for a certain period.
%The results of QoS evaluation are presented to verify that our proposed scheduling method meets the four requirements for service-centric multi-UAV search systems.まず、提案方式が utility を最大化できていることを示す
...
%次に、From a service-centric viewpoint で3つの評価指標を用いて評価を行ない、Benchmarkに対する有効性を示す
...
%UAV $i$は$N^i$枚の画像処理が終了すると即座に次の飛行を開始する．
...
\caption{Definition of notation}
 ...
 %$frac{d}{v^i}$ & Flying time per sensing section  \\ \hline
...
%ユーザはUAVの処理結果を取得するごとに効用が蓄積ができるものとし，その累積和を考える．
...
%いずれも時間の経過とともに単調増加しているが，提案方式は常に値が最大となっている．
%Cumulative sum of utilities monotonically increases in all the methods
...
%Service centric なシステム設計における、QoS指標を用いた評価の必要性を論じる。
...
\begin{figure}[t]
\begin{center}
\includegraphics[width= 7.5cm]{fig/twodim.png}
...
%We also adopts time requirement combining the above two types of elapsed times: Number of images whose elapsed time after acquired is less than a predetermined value when elapsed time from start time is a certain value.
